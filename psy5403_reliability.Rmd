...
# Data 
I am using the data set from `psych()` package.   
```{r rows.print = 10}
(big5 = psych::bfi)
```
...
```{r}
c("A", "C", "E", "N", "O") %>% 
  map(., ~cronbachs_alpha(big5, .)) %>%
  map_dfr("total", .id = "trait") %>% 
  # giving each row the name of the trait 
  mutate(trait = c("A", "C", "E", "N", "O")) %>% 
  select(trait, std.alpha)
```
Using the rule of thumb (Nunnally, 1978), all but neuroticism have *inadequate* internal reliabilities.   
```{r eval = FALSE}
# For a practical example computing alpha for one scale, as opposed to 5 as shown above. 
big5 %>% 
...
c("A", "C", "E", "N", "O") %>% 
  map_dfr(., ~{select(big5, num_range(., 1:5)) %>% 
      ...
      summarize(coefficient_h = (1 + (sum( (ML1^2) / (1 - ML1^2) )^-1) )^-1)}, .id = "traits") %>% 
...
big5 %>% 
  ...
  psych::glb.fa()
```
We see that the greatest lower bound of .87 is higher than the alpha and omega estimates.  
...
We can also create a tau-equivalent model by fixing factor loading of all neuroticism item with 1. 
```{r tau_equivalence}
neuroticism_te <- 'N =~ 1*N1 + 1*N2+ 1*N3 + 1*N4 + 1*N5'
